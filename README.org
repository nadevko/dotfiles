...
Overlay that provides all [[*Libraries][library]] functions to use with
~lib.extend~.
...
~trivial.loadDir :: (path -> any) -> path -> attrSet~ ::
  Read directory and make map of file names without extension to results of
  processing file paths in it.
